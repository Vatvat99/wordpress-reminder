...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    ...
    <footer class="entry-meta">
        ...
        <?php if ( get_post_type() == 'post' ) { ?>
            ...
            <?php $categories_list = get_the_category_list( __( ', ', 'mon_super_theme' ) ); ?>
            <?php if ( $categories_list && mon_super_theme_categorized_blog() ) { ?>
                <span class="cat-links">
                    <?php printf( __( 'Posted in %1s', 'mon_super_theme' ), $categories_list ); ?>
                </span>
            <?php } ?>

            <?php // Affichage des tags ?>
            <?php $tags_list = get_the_tag_list( '', __( ', ', 'mon_super_theme' ) ); ?>
            <?php if ($tags_list) { ?>
                <span class="sep"> | </span>
                <span class="tag-links">
                    <?php printf( __( 'Tagged %1$s', 'mon_super_theme' ), $tags_list ); ?>
                </span>
            <?php } ?>
        <?php } ?>
